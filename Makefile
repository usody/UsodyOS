...
# force build of Debian 11 bullseye
build_bullseye:
	VERSION_CODENAME='bullseye' ./build_UsodyOS.sh

# force build of Debian 12 bookworm
build_bookworm:
	VERSION_CODENAME='bookworm' ./build_UsodyOS.sh
...
boot_iso:
	sudo qemu-system-x86_64 \
		...
		-drive format=raw,file=iso/UsodyOS_2024.2.0-beta2.iso,cache=none,if=virtio
...
boot_iso_uefi:
	sudo qemu-system-x86_64 \
		...
		-enable-kvm -m 2G -vga qxl -netdev user,id=wan -device virtio-net,netdev=wan,id=nic1 \
		-drive format=raw,file=iso/UsodyOS_debug.iso,cache=none,if=virtio
...
boot_iso_uefi_secureboot:
	...
	sudo qemu-system-x86_64 \
		...
		-enable-kvm -m 2G -vga qxl -netdev user,id=wan -device virtio-net,netdev=wan,id=nic1 \
		-drive file=iso/UsodyOS_debug.iso,cache=none,if=virtio,format=raw,index=0,media=disk \
		-boot menu=on
